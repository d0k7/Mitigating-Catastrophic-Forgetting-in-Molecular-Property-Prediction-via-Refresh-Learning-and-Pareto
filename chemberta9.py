...
prev_model = None
train_oewc(model, bbbp_train_loader, bbbp_val_loader)

print("Testing on BBBP Dataset")
bbbp_test_accuracy, bbbp_test_auc, bbbp_test_report = evaluate(model, bbbp_test_loader)
print(f'Test Accuracy on BBBP: {bbbp_test_accuracy}, AUC: {bbbp_test_auc}')
print(f'Classification Report on BBBP:\n{bbbp_test_report}')

print("Starting training on Bitter Dataset")
train_oewc(model, bitter_train_loader, bitter_val_loader, prev_model=prev_model)

print("Testing on Bitter Dataset")
bitter_test_accuracy, bitter_test_auc, bitter_test_report = evaluate(model, bitter_test_loader)
print(f'Test Accuracy on Bitter: {bitter_test_accuracy}, AUC: {bitter_test_auc}')
print(f'Classification Report on Bitter:\n{bitter_test_report}')

print("Starting training on Sweet Dataset")
train_oewc(model, sweet_train_loader, sweet_val_loader, prev_model=prev_model)

print("Testing on Sweet Dataset")
sweet_test_accuracy, sweet_test_auc, sweet_test_report = evaluate(model, sweet_test_loader)
print(f'Test Accuracy on Sweet: {sweet_test_accuracy}, AUC: {sweet_test_auc}')
print(f'Classification Report on Sweet:\n{sweet_test_report}')

# Save the model after training
model.save_pretrained(dil_model_save_path)

# Section 2: OEWC + Refresh Learning
print("Starting training on BBBP Dataset with Refresh Learning")
model = initialize_model()
prev_model = None
train_oewc(model, bbbp_train_loader, bbbp_val_loader)
...
# Save the model after training with Refresh Learning
model.save_pretrained(dil_model_save_path)
