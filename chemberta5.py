...
# Model saving path for class incremental learning
save_model_path = '/scratch/sakshi.rs.cse21.itbhu/dheeraj/New_RL_Paper_2/CIL Saved Model'
...
epochs = 3  # Change this to 25 in the future for longer training
fisher_coeff = 1e-6
early_stopping_patience = 2  # Set the patience for early stopping

# Train the model with oEWC in a Class Incremental Learning (CIL) manner
def train_oewc_cil(model, train_loader, val_loader, fim=None, fisher_coeff=fisher_coeff, epochs=epochs, prev_model=None, save_model_name=None):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            early_stopping_counter = 0
            # Save the best model for this class task
            if save_model_name:
                model.save_pretrained(f'{save_model_path}/{save_model_name}')
                print(f"Model saved at {save_model_path}/{save_model_name}")
        else:
...
# Training and testing for Class Incremental Learning with OEWC and OEWC + Refresh Learning
def run_cil_training(model, fim, prev_model=None, method="OEWC"):
    print(f"\nSection: {method}")
    
    # Train on BBBP dataset (Class Increment 1)
    print("\n1) Train on BBBP Dataset (Class Increment 1)")
    prev_model = train_oewc_cil(model, bbbp_train_loader, bbbp_val_loader, fim=fim, prev_model=prev_model, save_model_name="bbbp_task_model")
    ...
    # Train on Bitter dataset (Class Increment 2)
    print("\n2) Train on Bitter Dataset (Class Increment 2)")
    prev_model = train_oewc_cil(model, bitter_train_loader, bitter_val_loader, fim=fim, prev_model=prev_model, save_model_name="bitter_task_model")
    print("Test on Bitter Dataset")
    test_accuracy, test_auc, test_report = evaluate(model, bitter_test_loader)
    print(f"Test Accuracy: {test_accuracy}, AUC: {test_auc}\nClassification Report:\n{test_report}")
    
    # Train on Sweet dataset (Class Increment 3)
    print("\n3) Train on Sweet Dataset (Class Increment 3)")
    prev_model = train_oewc_cil(model, sweet_train_loader, sweet_val_loader, fim=fim, prev_model=prev_model, save_model_name="sweet_task_model")
...
def run_oewc_cil():
    ...
    prev_model = run_cil_training(model, fim, method="OEWC")
    fim = init_fim(model)
    
    # Reinitialize the model for OEWC + Refresh Learning
    model = initialize_model()
    run_cil_training(model, fim, prev_model=prev_model, method="OEWC + Refresh Learning")

# Run the OEWC and OEWC + Refresh Learning in Class Incremental Learning format
run_oewc_cil()
