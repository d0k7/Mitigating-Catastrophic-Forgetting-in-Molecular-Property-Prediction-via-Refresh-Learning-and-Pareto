...
# Refresh Learning with DER++
def refresh_learning_with_derpp(dataset_name, train_loader, val_loader, test_loaders, epochs=3, lambda_er=0.01):
    ...
    # Initial training
    model = train_derpp(model, train_loader, val_loader, replay_buffer, optimizer, scheduler, criterion, epochs=epochs, lambda_er=lambda_er)

    # Unlearning phase
    print(f'--- Unlearning Phase ---')
    for k in range(epochs):
        print(f'Unlearning Phase {k + 1}/{epochs} ---')
        total_loss = 0
        model.train()
        for batch in train_loader:
            optimizer.zero_grad()
            input_ids, attention_mask, labels = batch
            output = model(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
            # Compute loss with replay buffer
            loss = output.loss + derpp_loss(replay_buffer, model, criterion, lambda_er)
            total_loss += loss.item()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
            ...
            scheduler.step()
        avg_train_loss = total_loss / len(train_loader)
        print(f'Epoch {k + 1}, Unlearning Loss: {avg_train_loss}')

        val_loss, val_accuracy, val_auc, val_report = evaluate(model, val_loader, return_loss=True)
        print(f'Validation Loss after Unlearning: {val_loss}, Accuracy: {val_accuracy}, AUC: {val_auc}')
        print(f'Classification Report for Validation after Unlearning:\n{val_report}')
    ...
    # Save the model
    save_path = f'/scratch/sakshi.rs.cse21.itbhu/dheeraj/New_RL_Paper_2/CIL Saved Model/DER++_{dataset_name}.bin'
    torch.save(model.state_dict(), save_path)
    print(f'Model saved to {save_path}')

# Execute DER++ + Refresh Learning on all datasets
test_loaders = {
    'BBBP': bbbp_test_loader,
    'Bitter': bitter_test_loader,
    'Sweet': sweet_test_loader
}

refresh_learning_with_derpp('BBBP', bbbp_train_loader, bbbp_val_loader, test_loaders)
refresh_learning_with_derpp('Bitter', bitter_train_loader, bitter_val_loader, test_loaders)
refresh_learning_with_derpp('Sweet', sweet_train_loader, sweet_val_loader, test_loaders)
